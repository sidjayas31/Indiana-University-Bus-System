create table spring_runs_det as select a.*,c.stop 'from_name',b.stop 'to_name' from (SELECT ival.*,wk.route,substring(wk.route,3,2) 'route_major',date(ival.when_time) 'time_date',time(ival.when_time) 'time_hour',
...
and date_format(wk.`Date`,'%Y/%m/%d') = date_format(ival.when_time,'%Y/%m/%d')AND subtime(date_format(when_time,'%h:%i:%S %p'),'4:00:00' ) BETWEEN subtime(`Clock In`,'4:00:00')
	AND subtime(`Clock Out`,'4:00:00')) where route is not null order by bus_id,route,when_time) a 
    left join DBMAP_StopID b on a.to_id=b.id
    left join DBMAP_StopID c on a.from_id=c.id order by route_major;

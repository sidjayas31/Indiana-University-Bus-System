...
FROM (	SELECT i.from_id, to_id,time_id, bus_id,r.`Route ID` 'route_id', when_time 
		...
		WHERE i.route_id=FLOOR(r.Index) and DATE(when_time) =str_to_date('2015/01/13','%Y/%m/%d') 
	)
	ival left join (select * from DBMAP_WorkRecord where `Date` = str_to_date('2015/01/13','%Y/%m/%d')  and `Shift Type` ='Standard'
	)wk 
